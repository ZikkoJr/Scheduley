...
public class SqliteCourseDao implements CourseDAO {
    ...
    public Optional<Course> findByCode(String code) {
        try (Connection conn = ConnectDB.getConnection();
             PreparedStatement ps = conn.prepareStatement(SQL_SELECT + " WHERE code = ?")) {
            ps.setString(1, code);
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) return Optional.of(map(rs));
                return Optional.empty();
    ...
    public List<Course> findAll() {
        ...
        try (Connection conn = ConnectDB.getConnection();
             PreparedStatement ps = conn.prepareStatement(SQL_SELECT + " ORDER BY code COLLATE NOCASE, name COLLATE NOCASE");
